...
/**
 * Public projection of matches (migration 007; `court` 010, `is_walkover` 011/013 → view v3): scores are NULL while a match
...
export const getPublicMatches = (sb, cols = '*') =>
  sb.from('matches_public_v3').select(cols).order('match_date').order('match_time');
