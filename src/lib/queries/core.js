...
/** All matches in schedule order (date, time). */
export const getMatches = (sb, cols = '*') => sb.from('matches').select(cols).order('match_date').order('match_time');

/** Bracket matches only (rows created by generate_bracket). */
export const getBracketMatches = (sb) => sb.from('matches').select('*').not('round', 'is', null).order('match_date').order('match_time');
...
export const getAnnouncements = (sb, { limit, orderBy = 'published_at' } = {}) => {
  let q = sb.from('announcements').select('*').order('is_pinned', { ascending: false }).order(orderBy, { ascending: false });
