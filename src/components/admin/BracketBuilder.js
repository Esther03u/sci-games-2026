          ...
          <form onSubmit={submit}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.75rem' }}>
              <FormField label="กีฬา" required id="br_sport">
                <select id="br_sport" className="form-input" value={sportId} onChange={(e) => setSportId(e.target.value)}>
                  <option value="">— เลือกกีฬา —</option>
                  {sportsWithout.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
                </select>
              </FormField>
              <FormField label="สนาม" id="br_venue">
                <input id="br_venue" className="form-input" placeholder="ว่างไว้ = ใช้ชื่อกีฬา" value={form.venue} onChange={set('venue')} />
              </FormField>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '0.75rem', marginTop: '0.4rem' }}>
              <FormField label="วันรอบรองฯ" id="semi_date"><input id="semi_date" type="date" className="form-input" value={form.semi_date} onChange={set('semi_date')} /></FormField>
              <FormField label="เวลารองฯ 1" id="semi_time_1"><input id="semi_time_1" type="time" className="form-input" value={form.semi_time_1} onChange={set('semi_time_1')} /></FormField>
              <FormField label="เวลารองฯ 2" id="semi_time_2"><input id="semi_time_2" type="time" className="form-input" value={form.semi_time_2} onChange={set('semi_time_2')} /></FormField>
              <FormField label="วันชิง" id="final_date"><input id="final_date" type="date" className="form-input" value={form.final_date} onChange={set('final_date')} /></FormField>
              <FormField label="เวลาชิงที่ 3" id="third_time"><input id="third_time" type="time" className="form-input" value={form.third_time} onChange={set('third_time')} /></FormField>
