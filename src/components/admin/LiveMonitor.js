...
import { relativeTime, fmtTime, fmtPlace } from '@/lib/format';
...
export default function LiveMonitor({ initial }) {
  ...
  const rows = useMemo(() => {
    const list =
      filter === 'active' ? matches.filter((m) => m.status !== 'finished' || isRecent(m, now)) : matches;
    ...
    <div>
      ...
      {rows.length === 0 ? (
        <GlassCard style={{ textAlign: 'center', padding: '2.5rem', color: 'var(--text-3)' }}>
          ไม่มีแมตช์ในมุมมองนี้ ·{' '}
          <Link href="/admin/matches" style={{ color: 'var(--gold-700)' }}>
            สร้างแมตช์
          </Link>
        </GlassCard>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
          {rows.map((m) => {
            ...
            const stale =
              m.status === 'live' &&
              m.last_score_at &&
              now - new Date(m.last_score_at).getTime() > 10 * 60 * 1000;
                  ...
                  {ev ? (
                    <>
                      ...
                      <div>
                        ...
                        {ev.event_type === 'score' && ev.team
                          ? ` ${ev.delta > 0 ? '+' : ''}${ev.delta} ${ev.team === 'a' ? a?.name : b?.name}`
                          : ''}
                        {' · '}
                  ...
                  {m.status === 'live' && (
                    <button
                      ...
                      onClick={async () =>
                        (await confirm({
                          title: 'จบแมตช์?',
                          message: `${sport?.name}: ${a?.name} ${isSets ? m.sets_a : m.score_a}–${isSets ? m.sets_b : m.score_b} ${b?.name}`,
                          confirmLabel: 'จบแมตช์',
                        })) && act(m, 'finish')
                      }
                    >
                      จบแมตช์
                  ...
                  {m.status === 'finished' && (
                    <button
                      ...
                      onClick={async () =>
                        (await confirm({
                          title: 'เปิดแมตช์ใหม่?',
                          message: 'แมตช์จะกลับเป็น "กำลังแข่ง" และคะแนนสะสมจะถูกคำนวณใหม่เมื่อจบอีกครั้ง',
...
function TeamPill({ team }) {
  return (
    ...
    >
      <span
        style={{ width: 8, height: 8, borderRadius: '50%', background: team?.color_hex || 'var(--border)' }}
      />
      {team?.name || 'รอผล'}
    </span>
  );
        ...
        {[
          ...
          <div key={t.label}>
            ...
            <input
              ...
              min="0"
              value={t.score}
              onChange={(e) => t.setScore(num(e.target.value))}
            />
            {isSets && (
              <>
                <label className="form-label" style={{ fontSize: '0.78rem', marginTop: '0.5rem' }}>
                  เซตที่ชนะ
                </label>
                <input
                  className="form-input"
                  ...
                  min="0"
                  value={t.sets}
                  onChange={(e) => t.setSets(num(e.target.value))}
                />
              </>
            )}
          </div>
          ...
          onClick={async () => {
            ...
            try {
              await onSubmit({
                score_a: scoreA,
                score_b: scoreB,
                sets_a: isSets ? setsA : null,
                sets_b: isSets ? setsB : null,
              });
            } finally {
              setSaving(false);
            }
