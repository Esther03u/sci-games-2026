...
export default function SlideCommit({
  ...
  // Measure actual width for responsive container support (e.g. width="100%")
  const isNumericWidth = typeof width === 'number' && width > 0;
  const [measuredWidth, setMeasuredWidth] = useState(() => (isNumericWidth ? width : 320));
  ...
  const effectiveWidth = isNumericWidth ? width : measuredWidth;
  ...
  const arrow = useTransform(
    [seen, shown],
    ([v, on]) => on * clamp(1 - (v - TRAVEL * 0.55) / (TRAVEL * 0.4), 0, 1)
  );
  ...
  return (
    <div
      ...
      style={{
        width: isNumericWidth ? `${width}px` : width || '100%',
        height,
        '--sc-track': trackColor,
        '--sc-ink': handleColor,
        '--sc-ok': successColor,
