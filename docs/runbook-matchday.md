...
1. **สร้าง PIN ให้กรรมการรายกีฬา** — `/admin/pins` → เลือกกีฬา + ตั้งชื่อ (เช่น "ฟุตซอล สนาม 1") → ระบบแสดง PIN + QR ให้แจกกรรมการ — **ดูซ้ำได้ที่ปุ่ม "ดู PIN"** ในตาราง (แอดมินเท่านั้น, บันทึกทุกครั้งที่เปิดดูใน `/admin/audit`)
...
| กรรมการลืม PIN | `/admin/pins` → ปุ่ม **ดู PIN** ของแถวนั้น (PIN ที่สร้างก่อนมีฟีเจอร์นี้ดูไม่ได้ → ปิดอันเก่าแล้วสร้างใหม่) · ถ้า PIN หลุดไปถึงคนอื่น → ปิดอันเก่าแล้วสร้างใหม่ |
