...
> Last updated: 2026-09-25 (**เพิ่มกลุ่มคำการ์ดลอยในแฟ้ม `<FolderFloat />` เป็น 14 รายการครอบคลุมทุกไฮไลต์เว็บและชนิดกีฬา รวมกลุ่มเป็นก้อนลอยในระบบฟิสิกส์ 2D Zero-Gravity** · **แก้ปัญหาการ์ดลอยบดบังหัวข้อหลัก "Sci Games 2026" ปรับระยะ Headroom และจัดเรียงกระชับ** · **คะแนนรวมตามอันดับที่ 1–4 + หน้าจับคู่สาขาเป็นการ์ดสี** — ตั้งคะแนนได้ที่ /admin/settings, ผู้ชมเห็นคะแนนรวมหลังเปิดโพเดียม · บูรณาการ Motion motion.dev ให้เว็บลื่นไหล สมูท ทุกจุด · 44/44 แมตช์ครบสมบูรณ์)
...
- ✅ **เพิ่มกลุ่มคำการ์ดลอยในแฟ้ม `<FolderFloat />` เป็น 14 รายการแบบกลุ่มก้อน (Particle Cluster) (25 ก.ย.)**:
  - พัฒนาตามความต้องการของผู้ใช้: เพิ่มคำสั้นๆ เกี่ยวกับเว็บและงานกีฬาให้เยอะๆ เป็นกลุ่มก้อนเลย
  - บรรจุข้อความไฮไลต์ 14 รายการ โดยไม่ใช้อิโมจิ:
    1. `9 – 11 ต.ค. 69` (กำหนดการแข่งขัน)
    2. `ฟุตซอล` (ชนิดกีฬา)
    3. `วอลเลย์บอล` (ชนิดกีฬา)
    4. `บาสเกตบอล` (ชนิดกีฬา)
    5. `เซปักตะกร้อ` (ชนิดกีฬา)
    6. `เปตอง` (ชนิดกีฬา)
    7. `รวมพลังความสามัคคี` (คำขวัญหลัก)
    8. `Live Score` (ระบบรายงานผลสด)
    9. `ตารางแข่งขัน` (ตารางแมตช์)
    10. `สูจิบัตร` (สูจิบัตรทางการ)
    11. `คณะวิทย์ PKRU` (คณะผู้จัด)
    12. `ม.ราชภัฏภูเก็ต` (มหาวิทยาลัย)
    13. `สรุปเหรียญรางวัล` (อันดับและเหรียญรางวัล)
    14. `4 สีประชันชัย` (ม่วง เขียว ชมพู เหลือง)
  - ปรับขนาดชิปการ์ดให้กะทัดรัด คมชัด พรีเมียม (`height: 30px`, `font-size: 11.5px`, `padding: 0 11px`, `rowHeight={32}`)
  - จัดเรียงเป็นกลุ่มก้อน 4 แถวในระบบ Matter.js Physics 2D ลอยและชนกันอย่างเป็นธรรมชาติในภาวะไร้น้ำหนัก (Zero Gravity Drift & Collision)
  - ผู้ใช้สามารถใช้นิ้วลาก โยน และโต้ตอบกับการ์ดแต่ละใบได้อย่างลื่นไหล
  - ควบคุมเพดานฟิสิกส์และ Headroom ปลอดภัย ไม่ลอยขึ้นไปบดบังหัวข้อ "Sci Games 2026"
