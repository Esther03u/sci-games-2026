...
- ✅ **ปรับโทนสีรอบชิงฯ และชิงอันดับ 3 เป็น Radiant Gold ทั้งหมด + แก้ไข Team Capsule ล้นกรอบ (24 ก.ย.)** — ตามความต้องการของผู้ใช้:
  ...
  - **แก้ไขปัญหาข้อความ "รอผลการแข่งขัน" ล้นกรอบการ์ด:** ปรับ grid template เป็น `minmax(0, 1fr) auto minmax(0, 1fr)` พร้อม `gap: 0.4rem`, ปรับขนาดกล่องเวลาตรงกลาง `minWidth: 84px` และขนาดแคปซูลทีม `fontSize: 0.78rem` พร้อม `maxWidth: 100%`, `overflow: hidden`, `textOverflow: ellipsis` ทำให้แคปซูลทั้งฝั่งซ้ายและขวาแสดงข้อความ "รอผลการแข่งขัน" ได้พอดีสวยงาม ไม่ถูกตัดขอบบนการ์ดขนาด ~330px
  - Prettier ผ่าน, `npm run build` ผ่าน 100%
