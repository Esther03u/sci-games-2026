...
> Last updated: 2026-09-25 (**เพิ่มกลุ่มคำการ์ดลอยในแฟ้ม `<FolderFloat />` เป็น 30 รายการแน่นๆ ทั้งคำคมปั่นๆ กีฬา และไฮไลต์เว็บ ทับซ้อนเป็นก้อนกลุ่ม (Dense Ball Cluster) พร้อมแก้ปัญหาชิดขอบจอด้วยการล็อกขอบข้างปลอดภัยมากกว่า 70px** · **แก้ปัญหาการ์ดลอยบดบังหัวข้อหลัก "Sci Games 2026" ปรับระยะ Headroom และจัดเรียงกระชับ** · **คะแนนรวมตามอันดับที่ 1–4 + หน้าจับคู่สาขาเป็นการ์ดสี** — ตั้งคะแนนได้ที่ /admin/settings, ผู้ชมเห็นคะแนนรวมหลังเปิดโพเดียม · บูรณาการ Motion motion.dev ให้เว็บลื่นไหล สมูท ทุกจุด · 44/44 แมตช์ครบสมบูรณ์)
...
- ✅ **เพิ่มกลุ่มคำการ์ดลอยในแฟ้ม `<FolderFloat />` เป็น 30 รายการแน่นๆ ทับซ้อนเป็นก้อนกลม + ล็อกไม่ให้ชิดขอบจอ (25 ก.ย.)**:
  - พัฒนาตามความต้องการของผู้ใช้: "มันชิดขอบไป และต้องการให้เยอะกว่านี้แบบทับซ้อนกันแบบเยอะๆ เลย หรือความคิดเห็นหรืออะไรก็ได้คำคมอะไรปั่นๆ อะไรได้หมดเลย แบบต้องการเป็นก้อนแน่นๆ เลย"
  - **แก้ปัญหาชิดขอบจอ**: ปรับคำนวณ `updateSpread` ให้เผื่อขอบจออย่างน้อย 96px (`window.innerWidth - 96`) และตั้ง `spread={120}` ทำให้คัตเอาต์ของกลุ่มก้อนกว้างเพียง ~240px เว้นขอบซ้าย-ขวาบนมือถืออย่างกว้างขวาง ปลอดภัย ไม่ชนขอบแดงแน่นอน
  - **ซ้อนทับกันเป็นก้อนแน่นๆ (Soft Overlap Matter Colliders)**:
    - ปรับขนาด Physics Body ของ Matter.js ให้เป็น `72%` ของขนาดชิป ทำให้การ์ดทั้ง 30 ใบเกยทับซ้อนกันได้ 28% เกิดมิติชั้นเลเยอร์ซ้อนกันอย่างหนาแน่นและสวยงาม
    - ใส่เส้นขอบชิปบาง `border: 1px solid rgba(0, 0, 0, 0.08)` ทำให้ชิปสีขาวที่เกยทับกันดูคมชัด มีมิติ ไม่กลืนกัน
  - **บรรจุชิปคำคมปั่นๆ + บรรยากาศงานแข่ง + ฟีเจอร์เว็บครบ 30 รายการ (ไม่มีอิโมจิ)**:
    - ข้อมูลงานและกีฬา: `9-11 ต.ค.`, `ฟุตซอล`, `วอลเลย์บอล`, `บาสเกตบอล`, `เซปักตะกร้อ`, `เปตอง`, `สีม่วง`, `สีเขียว`, `สีชมพู`, `สีเหลือง`, `Live Score`, `ตารางแข่ง`, `สูจิบัตร`, `คณะวิทย์ PKRU`, `สรุปเหรียญ`, `รวมพลังความสามัคคี`
    - คำคมปั่นๆ และเสียงเชียร์: `ซ้อมมาทั้งปีเพื่อวันนี้`, `ไม่เน้นแข่ง เน้นเชียร์`, `หลีดซ้อมถึงตีสอง`, `แต้มไหลเป็นน้ำ`, `พร้อมบวกทุกสนาม`, `ช็อตฟีลฝั่งตรงข้าม`, `แชมป์อยู่ตรงนี้`, `พักผ่อนคืออะไร`, `เสียงแหบแน่นอน`, `อาจารย์อย่าเช็คชื่อ`, `เจอกันที่โพเดียม`, `ของแทร่`, `กองเชียร์อันดับ 1`, `สู้สุดใจ`
  - ปรับความสูง `rowHeight={21}` และ `margin-top: 10.25rem` บนมือถือ ปลอดภัยใต้หัวข้อ "Sci Games 2026"
